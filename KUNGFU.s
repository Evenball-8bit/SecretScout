...
;
.feature force_range
	REPRATE = 4		;KEY REPEAT RATE IN 1/60 SECONDS.  DON'T 
				;MONKEY WITH THIS!

	J1REPENA = %00001111	;REPEAT ENABLE FLAG BITS FOR EACH J1 BUTTON
	J2REPENA = %00001111	;REPEAT ENABLE FLAG BITS FOR EACH J2 BUTTON
...
;SOUND SLOT BUT NOTHING ELSE.
	SPSLOTS = 16		;MAX ALLOWED ACTIVE SPRITE BLOCKS
	SNDSLOTS = 5		;MAX ALLOWED ACTIVE SOUND STRUCTURES
				;TOTAL=211 BYTES
ROMVER = -1
...
;0-3 ARE THE MUSIC SLOTS 0-3
	...
	DINGSLOT = 4		;FOR THE PAUSE DING OR NOISE COMMAND
...
;		IN PARAM2 (WORLD TO GO TO, ETC.):
				DOOR = 0		;USE FOR AN OPENING
				CLIMB = 1		;USE FOR CLIMBING (LADDERS)
				BARRIER = 2	;USE FOR BARRIERS = FORCE FIELDS
				SINK = 3		;USE FOR SWAMPS =  QUICKSAND
				TUNNEL = 4	;USE FOR TUNNEL (HORIZONTAL)
				PIPE = 5		;USE FOR PIPE (VERTICAL)
				BEAM = 6		;TRANSPORTER (BEAM ME UP!)
				NEWWORLD = 7	;GO TO NEW WORLD.  PUT NEW
				ELAUNCH = 8	;PLACE THAT LAUNCHES ENEMY
				TRAP = 9		;BOOBY TRAP THAT DOES ???
				TRANSFORM = 10	;TO TRANSFORM THE ENTITY.
				ENDSCENE = 11	;FOR END SCENE REQUEST
				WARPZONE = 12	;WARP ZONE ENTRANCE
				PORTAL = 13	;DOOR TO ANOTHER WORLD
...
;REMEMBER!!! THESE MUST NOT OVERLAP WITH THE VALUES USED BY KUNGSUBS.ASM.
	...
	WORLD_REQUEST = $00	;SET TRUE TO REQUEST A NEW WORLD.
	CURWORLD = $01		;CURRENT WORLD (VALID IF WORLD_REQUEST=0)
	WORLD_INDX = $02		;CURRENT WORLD AS AN INDEX (0,2,4,6,ETC).

	SAFEX = $03		;FOR SAVING X VALUES.
	SAFEY = $04		;FOR Y VALUES.
	SAFEX2 = $05		;A SECOND PLACE TO SAVE X 
	SAFEY2 = $06		;SAME FOR Y.

	RESETFLG = $07		;SET TRUE AT RESET FOR THE RESET COMMAND

	SILENCEIT = $08		;USED BY SELECT MODE TO SILENCE NEW WORLDS.

	TIMETICK = $09		;LOADED WITH CLOCKPHASE.  WHEN IT DECS TO
				;ZERO PHASE4 IS SET.
				;AND USED BY MANY ROUTINES FOR PACING.

	SPECGOTO = $0A		;TRUE IF SPECIAL WORLDGOTO CONDITION

	LEFTCOL = $0B		;LEFTMOST COLUMN TO HAVE IN THE GOTO WORLD
;		$0C		;OR WHEN SAVING A WORLD'S STATE.

	CLOCKP = $0D 		;CLOCKPHASE THATS USER MODIFIABLE.

	STARTCOL = $0E		;LEFTMOST COLUMN OF SCREEN WITHIN
;		   0F		;CURRENT BACKGROUND.

	SAFEBYTE = $10		;TEMPORARY BYTE FOR USE FROM BOTH INSIDE
				;AND OUTSIDE NMI

	SAFEWORD = $11		;TEMPORARY WORD FOR USE FROM OUTSIDE NMI.
;		 $12		;EVEN THOUGH ITS IN LOW RAM, DON'T USE IT
				;FOR INDIRECT SO THAT WE CAN TRANSLATE
				;THIS CODE TO OTHER MACHINES MORE EASILY.

	SCROLLFLG = $13		;SCROLL FLAG.  BIT ORIENTED:
				;08 = UP SCROLL DISABLED
				;04 = DOWN SCROLL DISABLED
				;02 = LEFT SCROLL DISABLED
				;01 = RIGHT SCROLL DISABLED

	PHASE4 = $14		;SET BY USER_NMI TO SIGNAL ITS TIME TO MOVE
				;ENEMIES.  BIT ORIENTED:
				;01 = TIME FOR NEW CLOCK PHASE (MOVE ENEMIES)
				;80 = LAST MOVEMENT CYCLE GOT BEHIND.

	SELKEY = $15		;WHAT THE SELECT BUTTON SHOULD DO
;	     $16		;-1 IS NO INTERRUPT

	NEWCOL = $17		;USED BY JOYSTICK ROUTINE TO FLAG
				;THAT A JOYSTICK ENTITY NEEDS TO TEST
				;A COLUMN AGAIN.

	PAUSEFLG = $18		;USER CAN DISABLE PAUSE HERE.  
				;DEFAULT=DISABLED

	RNDNUM1 = $19
	RNDNUM2 = $1A
	XRANDOM = $1B		;FOR RANDOM NUMBER GENERATOR

	WORLDCOL = $1C		;COLUMN WITHIN BACKGROUND PASSED TO SCREENFLGS
;		   1D

	RNDPTR = $1E		;FOR THE RANDOM NUMBER GENERATOR
;		 1F

	ALTPHASE = $20		;INCREMENTED EACH PHASE FOR HALF SPEED
				;ENTITIES

	NUMBUF = $20 		;BUFFER TO HOLD VIDEO WRITE DATA FOR CONVNUM
;	       $24 

	NMICOL = $25		;HOLDS TESTCOL FOR USE WHEN NEEDCOL IS SET.
;	       $26

	SAVEDWORLD = $27 	;FOR STATREQUEST COMMAND.

	BPAUSE = $28		;SET TRUE WHEN START IS PRESSED TO FORCE PAUSE.

	FLAGBYTE = $29		;THE FIRST OF 6 BYTES USED WITH THE HERO 
	BYTE1 = $2A		;WORLD COMMAND THAT IS CURRENTLY BEING RUN
	BYTE2 = $2B
	BYTE3 = $2C
	BYTE4 = $2D
	BYTE5 = $2E

	QUARTERYLOST = $2F	;AMOUNT OF GLOBAL_YSCR LOST BY THE 1/4ers
	HALFYLOST = $30		;AMOUNT LOST BY THE 1/2 SCROLLERS.  WE ADD
				;IT UP AND USE IT EACH TIME.
	GLOBAL_YSCR = $31	;SET TO DIRECTION TO SCROLL ENTITY AS A RESULT
				;OF JOYSTICK CONTROLLED SCREEN SCROLL.
	HALF_YSCR = $32		;CALCULATED FOR THOSE THAT USE 1/2
	QUAR_YSCR = $33		;FOR THOSE THAT USE 1/4

	QUARTERXLOST = $34	;AMOUNT OF GLOBAL_XSCR LOST BY THE 1/4ers
	HALFXLOST = $35		;AMOUNT LOST BY THE 1/2 SCROLLERS.  WE ADD
				;IT UP AND USE IT EACH TIME.
	GLOBAL_XSCR = $36	;SET TO DIRECTION TO SCROLL ENTITY AS A RESULT
				;OF JOYSTICK CONTROLLED SCREEN SCROLL.
	HALF_XSCR = $37		;CALCULATED FOR THOSE THAT USE 1/2
	QUAR_XSCR = $38		;FOR THOSE THAT USE 1/4

	MOMINDX = $39		;JOYSTICK MOMENTUM INDEX
	JOYINDX = $3A		;JOYSTICK INDEX. 0 OR 1

	ENABLE_SELECT = $3B	;USED TO ENABLE WORLD SELECT MODE
...
;OF 0 OR 1 TO STORE MOMENTUM OR DECIDE WHICH ENEMY TO APPLY IT TO.
	...
	JOYENEMY = $3C		;ENEMY INDEX ATTACHED TO THIS JOYSTICK.
;		 $3D		;-1 = NONE.

	JOYX = $3E		;ACCUMULATED X MOMENTUM FOR THIS JOYSTICK
;	     $3F

	JOYY = $40		;ACCUMULATED Y MOMENTUM FOR THIS JOYSTICK
;	     $41

	SCOREL = $42		;LOW BYTE OF SCORE FOR JOYSTICK 0 AND 1
;	       $43		;SCOREH MUST FOLLOW THIS!
	SCOREH = $44		;HIGH BYTE OF SCORE FOR JOYSTICK 0 AND 1
;	       $45		

	HEROCOUNT = $46		;COUNT OF HEROS LEFT FOR EACH JOYSTICK
;		  $47

	PASSCNT = $48		;USED BY MOVEENEMY TO ALLOW 3 COMMANDS PER
				;TICK.  RESET BY ANY COMMAND THAT CHANGES
				;MOMENTUM.

	SAVED1 = $49		;USED TO SAVE STUFF FOR INITSTAT SO THAT
	SAVED2 = $4A		;ENDREQUEST CAN RESTORE IT.
	SAVED3 = $4B
	SAVED4 = $4C

	SAVESEL = $4D		;TO SAVE WHAT SELECT DOES IF STATREQUEST WORLD
;		$4E

	STARTROW = $4F		;TOPMOST ROW ON SCREEN (USED IN BLOCK MODE)

	TOPROW = $50		;TOPMOST ROW TO HAVE IN THE GOTO WORLD
				;OR WHEN SAVING A WORLD'S STATE.

	TABLEPTR = $51		;POINTER TO THE SCREEN COLUMN FLAGS FOR
;		 $52		;THE CURRENT WORLD.  BOTH NMI AND NON-NMI
				;USE THIS BUT NMI SAVES IT OUT.

	ENEMYPTR = $53		;ENEMY POINTER FOR USE BY MOVEENEMY
;		   54

	EXTRA_PTR = $55		;INDIRECT POINTER FOR NON NMI USE (ALL USERS)
;		  $56

	SETUPFLG = $57		;USED BY SETUPANIM.

	CHEATMODE = $58		;USED TO INFORM CODE THAT CHEAT MODE ENTERED
				;THE CURRENT WORLD.

	TEST_JMP = $59		;INDIRECT JUMP FOR HERO FLAGS TESTING.  ALSO
;		 $5A		;USED AS AN INDIRECT POINTER BY SAME.  CAN
				;BE USED FROM OUTSIDE NMI LEVEL BY ANYONE.

	SAVEJOY = $5B		;TO SAVE JOYENEMY FOR STATREQUEST
;		$5C
		    
	NEEDCOL = $5D		;SET TRUE WHEN WE SCROLL THE BACKGROUND IF
				;WE NEED TO TEST A NEW SCREEN COLUMN.  ITS
				;SET BY NMI FOR NON-NMI TO USE.

	TESTCOL = $5E		;THE COLUMN WE NEED TO TEST WHEN CALLING
;		  5F		;SCREENFLAGS ROUTINES.  MUST ONLY BE SET 
				;OUTSIDE NMI.

	CMDBUF = $60		;BUFFER TO HOLD DATA FOR COMMANDS UP TO 16 BYTES.
	DATA1 = $61
	DATA2 = $62
	DATA3 = $63
	DATA4 = $64		;THESE ARE HERE TO MAKE DIRECT ACCESS TO
	DATA5 = $65		;PARAMETERS EASIER
	DATA6 = $66
;	       $6F		

	ENEMYLREG = $70			;REGISTER LOW.  ALSO USED 
					;TO HOLD THE COLUMN OF LAUNCH WHILE
					;AN ENEMY IS SLEEPING.
	ENEMYHREG = $80			;REGISTER HIGH.  ALSO USED TO
...
;100-11F FOR USER SUBS TO USE.
	...
	REMEMBER = $120		;PLACE TO REMEMBER UP TO 1024 ENEMIES DEATH.
;		 $13F

	STATERAM = $140
;		   15F		;32 BYTES TO HOLD 1 WORD FOR EACH ENTITY

	COMMONDATA = $160	;20H BYTES OF COMMON DATA FOR DATAAREA
;		   $17F	;COMMANDS.  USUALLY ITS FOR PRIZES.

;*************** ENTITY VARIABLES PLACED AT AVAILABLE SPOTS.  ************

	ENEMYLCOL = $90		;WORLD COLUMN FLAG THAT LAUNCHED THE ENEMY.
	ENEMYHCOL = $A0		;WORLD COLUMN FLAG THAT LAUNCHED THE ENEMY.
	ENEMYFEET = $B0		;ENEMIES FEET

	ENEMYCTRL = $C0		;CONTROL FLAG FROM ENEMY LAUNCH
;		    CF

	ENEMYLINK = $D0 		;ENEMY TO WHOM THIS ENEMY IS LINKED.  
				;-1 = NONE

	ENEMYCNT = $360		;ENEMY COUNTER #1
	ENEMYALT = $370		;ENEMY COUNTER #2

;********* ENEMY VARIABLES PLACED CONTIGUOUSLY. **************
;
	EBASE = $4D0
	ECNT = 16			;NUMBER OF ENEMIES ALLOWED. ACTUALLY YOU CAN'T
	...
	ENEMY0 = EBASE		;-1 IF INACTIVE, NOT -1 IF ENEMY IS ACTIVE.  
	...
	ENEMYREQ = EBASE+ECNT*1  	;ENEMY REQUESTS.  SET EXTERNALLY TO REQUEST
	...
	ENEMYXMOM = EBASE+ECNT*2	;ENEMY'S X MOMENTUM FOR ENEMIES 0-5.
				;ITS THE MOMENTUM/8 AS FOR THE HERO

	ENEMYYMOM = EBASE+ECNT*3	;ENEMY'S Y MOMENTUM FOR ENEMIES 0-5.
				;ITS MOMENTUM/8 AS FOR THE HERO.

	ENEMYHEALTH = EBASE+ECNT*4 ;AMOUNT OF HEALTH LEFT IN THE ENEMY.

	ENEMYDELAY = EBASE+ECNT*5	;SET TO DELAY ENEMY WITH THE DELAY COMMAND.
				;WILL STILL MOVE ACCORDING TO MOMENTUM
				;AND GRAVITY.

	ENEMYATK = EBASE+ECNT*6	;LAST ENEMY THAT ATTACKED THIS ENEMY. ITS
				;THE SLOT NUMBER, NOT THE ENTITYTAB
				;ENTRY.  -1 = NONE.

	ENEMYDAMG = EBASE+ECNT*7	;DAMAGE AND ATTACK NUMBER OF LAST ATTACKER
				;07H = DAMAGE REQUESTED
				;08H = REPORT ATTACK NUMBERS 16-31
				;F0H = ATTACK NUMBER REQUESTED.

	ENEMYHIGH = EBASE+ECNT*8	;ENEMY HEIGHT FROM THE ANIMATION OR SPRITE.
				;ITS HIS PIXEL HEIGHT.

	ENEMYWIDE = EBASE+ECNT*9	;ENEMY WIDTH FROM THE ANIMATION OR SPRITE.
				;ITS HIS PIXEL WIDTH.

	ENEMYX = EBASE+ECNT*10	;CURRENT ENEMY X LOCATION

	ENEMYY = EBASE+ECNT*11	;CURRENT ENEMY Y LOCATION
		
	ENEMYLROW = EBASE+ECNT*12	;ENEMIES LAST ROW THAT FLAGS TESTED ON

	ENEMYINL = EBASE+ECNT*13	;LOW BYTE OF WORD INDEX INTO MEMORY WHERE 
				;CURRENT MOVEMENT TABLE IS BEING RUN.

	ENEMYINH = EBASE+ECNT*14	;HIGH BYTE OF SAME INDEX.

	ENEMYFLGS = EBASE+ECNT*15	;ENEMY FLAGS FROM ENEMY TABLE AT LAUNCH TIME

	ENEMYGRAV = EBASE+ECNT*16	;ENEMY Y AXIS GRAVITY

	ENEMYXLIM = EBASE+ECNT*17	;ENEMY X MOMENTUM LIMIT

	ENEMYYLIM = EBASE+ECNT*18	;ENEMY Y MOMENTUM LIMIT

	ENEMYSOLID = EBASE+ECNT*19	;ENEMY SOLID VALUE

	ENEMYCENTR = EBASE+ECNT*20 ;CURRENT ENEMY CENTER OF BODY X LOCATION

	ENEMYJSTAT = EBASE+ECNT*21	;SOME MORE ENEMY STATUS BITS.	
	...
	ENEMYTLLOW = EBASE+ECNT*22 ;ENTITY'S TARGET LEVEL, LOW BYTE
	ENEMYTLHI = EBASE+ECNT*23  ;ENTITY'S TARGET LEVEL, HIGH BYTE

	ENEMYTRACK = EBASE+ECNT*24 ;TARGET TRACKING DIRECTION AND ENTITY
				 ;CURRENT DIRECTION:
	...
	ENEMYTRLOW = EBASE+ECNT*25 ;ENTITY'S TARGET, LOW BYTE
	ENEMYTRHI = EBASE+ECNT*26  ;ENTITY'S TARGET, HIGH BYTE

	ENEMYD1 = EBASE+ECNT*27	;DAMAGE FLAG #1
	ENEMYD2 = EBASE+ECNT*28	;DAMAGE FLAG #2
	ENEMYD3 = EBASE+ECNT*29	;DAMAGE FLAG #3
	ENEMYD4 = EBASE+ECNT*30	;DAMAGE FLAG #4
	ENEMYD5 = EBASE+ECNT*31	;DAMAGE FLAG #5


	ENEMYRSL = EBASE+ECNT*32	;RESUME COMMAND PTR LOW
	ENEMYRSH = EBASE+ECNT*33	;RESUME COMMAND PTR HIGH. -1 = NO RESUME POINT

	ENEMYSTAT = EBASE+ECNT*34	;ENEMY CONTROL FLAGS FOR SPECIFIC STATUSES:
	...
	ENEMYXFRICT = EBASE+ECNT*35	;ENEMY X FRICTION OR OFFSET TO USE
					;ON X AXIS WHEN LINKED.
	ENEMYYFRICT = EBASE+ECNT*36	;ENEMY Y FRICTION OR OFFSET TO USE
					;ON X AXIS WHEN LINKED.

	ENEMYBOUNCE = EBASE+ECNT*37	;ENEMY BOUNCE FACTOR.

	ENEMYSD = EBASE+ECNT*38		;SAVED ENEMYDELAY BEFORE EXTERNAL 
					;EVENT THAT CAN BE RESUMED.  

	ENEMYFCTR = EBASE+ECNT*39		;FRICTION COUNT DOWN.
					;F0 = COUNT SINCE LAST Y MOM APPLIED
					;0F = COUNT SINCE LAST X MOM APPLIED

	ENEMYID = EBASE+ECNT*40		;ID OF LAST ATTACKER (ENTITYTAB
					;ENTRY NUMBER).  VALID ONLY IF
					;ENEMYATK IS NOT -1.  LOADED WITH
					;-1 FOR USE BY HURTING DISABLED
					;ENTITIES.

	ENEMYP1 = EBASE+ECNT*41		;FIRST BYTE PASSED BY WORLD EVENT.
	ENEMYP2 = EBASE+ECNT*42		;SECOND BYTE PASSED BY WORLD EVENT.


	ENEMYINTS = EBASE+ECNT*43		;ENEMY INTERRUPTS DISABLE FLAG BIT
					;THE INTERRUPT BITS ARE DOCUMENTED
					;IN THE DISABLE COMMAND IN KUNGTAB
	ENEMYSINT = EBASE+ECNT*44		;SAVED ENEMY INTERRUPTS FOR RESUME

	ENEMYNUM = EBASE+ECNT*45		;ENEMY'S NUMBER IN THE PERMANENT MEMORY.

	ENEMYVCTM = EBASE+ECNT*46		;ENEMY'S LAST VICTIM

	ENEMYLTST = EBASE+ECNT*47		;LAST COLUMN THAT THE ENEMY TESTED.
	ENEMYHTST = EBASE+ECNT*48		;AND ALSO A HIGH BYTE FOR 1 WORD.

	ENEMYFACE = EBASE+ECNT*49		;ENEMY DIRECTION FLAG.  SEE KUNGTAB.


	ENEMYBEHAVE = EBASE+ECNT*50	;ENEMIES PERSONALITY BITS
...
.ifdef BLOCKMODE ;.ifdef BLOCKMODE ;IFDEF( `BLOCKMODE', `		;BLOCK MODE HAS A Y STARTING OFFSET
	LDA	SUBSVAR2
	JSR	TIMES8
	STA	SUBSVAR2 
.endif
...
.ifdef BLOCKMODE ;.ifdef BLOCKMODE ;IFDEF( `BLOCKMODE', `		
	STA	GLOBAL_YSCR	
	STA	HALFYLOST
	STA	QUARTERYLOST
	.endif
...
FINDLOCS:			
	LDX	SAFEBYTE
	JSR	FINDCENTR	;This was spelled wrong
	JSR	FINDFEET
	RTS
